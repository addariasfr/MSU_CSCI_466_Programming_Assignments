...
class NetworkPacket:
    ...
    def from_byte_S(self, byte_S):
        ...
        priority = byte_S[NetworkPacket.dst_S_length + NetworkPacket.src_S_length : NetworkPacket.dst_S_length + NetworkPacket.src_S_length + NetworkPacket.priority_length]
        data_S = byte_S[NetworkPacket.dst_S_length + NetworkPacket.src_S_length + NetworkPacket.priority_length : ]        
        return self(dst, src, data_S, priority)
...
class MPLSFrame:
    ...
    # @param priority: the forwarding priority of the packet
    # @param packet_load: packet payload
    def __init__(self, label, priority, packet_load):
        self.label = label
        self.priority = priority
    ...
    def from_byte_S(self, byte_S):
        ...
        priority = byte_S[self.label_S_length : self.label_S_length + self.priority_length]
        packet_load = byte_S[self.label_S_length + self.priority_length:]
        return self(label, priority, packet_load)

## Implements a network host for receiving and data
class Host:
...
class Router:
    ...
    def process_queues(self):
        for i in range(len(self.intf_L)):
            ...
            #decapsulate the packet each packet
            p_to_process = None
            p_type = None
            for fr_S in list(self.intf_L[i].in_queue.queue):  # for each packet in the queue
                fr = LinkFrame.from_byte_S(fr_S)  # extract the link frame
                pkt_S = fr.data_S  # get the data from the link frame
                #process the packet as network, or MPLS
                if fr.type_S == "Network":
                    p = NetworkPacket.from_byte_S(pkt_S) #parse a packet out
                    if p_to_process is None:  # if it is the first packet
                        p_to_process = p  # set it to be processed
                        p_type = fr.type_S  # set the type of the packet
                    elif p.priority > p_to_process.priority:  # if it has higher priority
                        p_to_process = p  # set it to be processed
                        p_type = fr.type_S  # set the type of the packet
                elif fr.type_S == "MPLS":
                    m_fr = MPLSFrame.from_byte_S(pkt_S)  #parse a frame out
                    if p_to_process is None:  # if it is the first packet
                        p_to_process = m_fr  # set it to be processed
                        p_type = fr.type_S  # set the type of the packet
                    elif m_fr.priority > p_to_process.priority:  # if it has a higher priority
                        p_to_process = m_fr  # set it to be processed
                        p_type = fr.type_S  # set the type of the packet
                if p_type == "Network":
                    self.process_network_packet(p_to_process, i)
                elif p_type == "MPLS":
                    self.process_MPLS_frame(p_to_process, i)  #send the MPLS frame for processing
                else:
    ...
    def process_network_packet(self, pkt, i):
        for src in self.encap_tbl_D:  # check each entry in the encapsulation table
            if src == pkt.src:  # if the table entry matches the packet source
                m_fr = MPLSFrame(self.encap_tbl_D[src], pkt.priority, pkt.to_byte_S())  # create an MPLS table with the given label
        ...
        for lbl in self.frwd_tbl_D:  # check each label in the forwarding table
            if lbl == m_fr.label:  # if the table entry matches the packet label
                ...
                if new_lbl == '':  # if the label is a last hop label
                    ...
                    fwd_pkt = MPLSFrame(new_lbl, m_fr.priority, m_fr.packet_load)  # create the MPLS packet with the new label
                    pkt_type = "MPLS"
