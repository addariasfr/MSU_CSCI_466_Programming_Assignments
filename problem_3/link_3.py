...
class Link:
    ...
    def tx_pkt(self):
        from network_3 import NetworkPacket,  MPLSFrame
         ...
         (self.node_2, self.node_2_intf, self.node_1, self.node_1_intf)]: 
            ...
            try:
                ...
                if intf_a.next_avail_time <= time.time():
                    ...
                    pri0_count = 0
                    pri1_count = 0
                    for packet in list(intf_a.out_queue.queue):
                        link_frame = LinkFrame.from_byte_S(packet)
                        if link_frame.type_S == "Network":
                            network_packet = NetworkPacket.from_byte_S(link_frame.data_S)
                        elif link_frame.type_S == "MPLS":
                            mpls_frame = MPLSFrame.from_byte_S(link_frame.data_S)
                            network_packet = NetworkPacket.from_byte_S(mpls_frame.packet_load)
                        if network_packet.priority == '0':
                            pri0_count += 1
                        elif network_packet.priority == '1':
                            pri1_count += 1
                    print('%s: transmitting frame "%s" on %s %s -> %s %s \n' \
                          ' - seconds until the next available time %f\n' \
                          ' - queue size %d\n' \
                          ' - contains: priority 0: ( %d ), priority 1: ( %d )'
                          % (self, pkt_S, node_a, node_a_intf, node_b, node_b_intf, intf_a.next_avail_time - time.time(), intf_a.out_queue.qsize(), pri0_count, pri1_count))
